...
void BotConfig::Read()
{
  ...
  this->MaxMessageLen        = this->Parser->GetInteger("Bot", "MaxMessageLength", 510);
  ...
  this->SockWait             = this->Parser->GetInteger("Bot","Socket Timeout",5);
  this->LogTime              = this->Parser->GetInteger("Log", "Log Time", 0);
  this->ServerPassword       = this->Parser->Get("Connect", "Password", "");
