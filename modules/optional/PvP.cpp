...
namespace BattleField
{
	...
	bool PlayerInField(Flux::string PlayerName)
	{
		if(BattleField::Players.find(PlayerName) != BattleField::Players.end()) return true;
		else return false;
	}
...
public:
	...
	void Run(CommandSource &Source, const std::vector<Flux::string> &params) override
	{
		...
		{
			Source.Reply("You are already in the game.");
		}
	}
};
...
public:
	...
	void Run(CommandSource &Source, const std::vector<Flux::string> &params) override
	{
		if((params[1] == "me") ^(params[1] == "myself") ^(params[1] == Source.u->nick))
		{
			...
			{
				Source.Reply("Sorry, but you aren't in the game.");
			}
		}
		else
		{
			Source.Reply("Number of players: %i", BattleField::NumberOfPlayers());
		}
	}
};
