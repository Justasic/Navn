 ...
 */
void world_clock(Socket &sock, irc_string *reply, Flux::string rply){
  if (reply->said("!time") && in_channel){
	...
	if(!irc_string::said(xmlToString("temp.xml"),"problem_cause")){
	  ...
	  if(loc == "Error: length too large"){
	    sock << privmsg(chan, "flux_net_irc::findInXML returned error: length too large.");
	    log("%s requested !time for %s and got error %s", unick.c_str(), location.c_str(), loc.c_str());
	  }else{
